...
**Act 3 — Judgment.** Two workshops. The first integrates the foundations by tracing a real request through every layer. The second applies the topical depth by reviewing AI-generated pull requests for production fitness.
...
The foundation arc is the conceptual scaffolding the rest of the course assumes. If you are going to spend any time at all on this material, this is the part that compounds.
...
Each module stands alone. Jump to the module that matches your current problem — there is no rule that says you have to read in order once the foundations are in place.
...
Plan for the conversations, not just the reading. The shared-vocabulary outcome — the one that makes PR reviews and incident calls move faster because everyone is using the same words for the same things — only develops through conversation. Reading alone is not enough.
