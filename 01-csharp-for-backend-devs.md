...
There is a layer of programming where AI produces fluent output, and a layer where fluent output isn't enough. AI handles syntax well. What requires a human is judgment about whether the choices that look fluent are the right choices for this code, in this system, on this team. Answering that requires reading at the level of mechanism rather than syntax: knowing what each language feature does at runtime, what each abstraction commits the system to, and where each guarantee starts and stops.
...
The first place language fluency is exercised under pressure is the code review of an AI-generated pull request. AI tools produce code that looks idiomatic — the keywords are right, the patterns are recognizable, the conventions are followed. Whether the code is doing what it appears to be doing is a separate question, and answering it requires reading at the level of mechanism rather than at the level of syntax.
